...
function sysIni(data){
	...
	now = nowDate.getTime();
	// 协议附件 start
	attach1 = data.Attach1;
	attach2 = data.Attach2;
	attach3 = data.Attach3;
	// 协议附件 end
...
/* 2020-3-8 0008
* 作者：王青高
* 功能：{}login.html 页面上的本次开盘时间
* 参数：{}
*/
function openDateTime() {
	let _startTime = dayjs(StartTime).format('YYYY年MM月DD日HH:mm');
	let _hour = dayjs(StartTime).format('HH');
	let _endTime = '';
	if (ShoppingTime < (60*60*24)) {
		_endTime = dayjs(StartTime).add(ShoppingTime, 'second').format('HH:mm');
	} else {
		_endTime = dayjs(StartTime).add(ShoppingTime, 'second').format('YYYY年MM月DD日HH:mm');
	}
	$('.tips_time').html(_startTime+'-' + _endTime);
	$('._hour').html(_hour);
}
...
*/
function attachment() {
	$('.tips_attachment .txt').each(function(index, ele) {
		index +=1;
		switch (index) {
			case 1:
				$(this).attr('data-src', attach1);
				break;
			case 2:
				$(this).attr('data-src', attach2);
				break;
			case 3:
				$(this).attr('data-src', attach3);
				break;
			default:
				$(this).attr('data-src', attach1);
		}
